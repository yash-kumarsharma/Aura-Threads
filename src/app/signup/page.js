...
export default function SignupPage() {
  ...
  const handleSubmit = (e) => {
    ...
    const result = register(name, email, password, location, "customer");
    if (result.success) {
      setSuccess(true);
      setTimeout(() => {
        router.push("/login");
    ...
    <main className="min-h-[85vh] flex items-center justify-center p-6 bg-gradient-to-br from-[#FAF9F6] to-[#F0EFEA] py-12 sm:py-20">
      <div className="bg-white border border-black/5 rounded-none overflow-hidden shadow-2xl max-w-5xl w-full min-h-[600px] grid grid-cols-1 md:grid-cols-12 g-0">
        ...
        <div className="md:col-span-5 text-white p-8 sm:p-12 flex flex-col justify-center relative select-none overflow-hidden min-h-[300px] md:min-h-auto">
          {/* Background Image */}
          <img 
            src="/images/campaigns/auth-customer.jpeg" 
          ...
          <div className="relative z-20">
            <span className="text-white/80 font-bold uppercase tracking-[0.25em] text-[10px] mb-2 block font-sans">Join Today</span>
            <h2 className="text-2xl font-black text-white mb-4 tracking-widest font-sans uppercase">AURA THREADS</h2>
            <p className="text-white/80 text-xs leading-relaxed mb-8 font-sans max-w-xs">Create an account to track shipments, rate collections, and list custom designs as a verified merchant.</p>
            <div>
        ...
        <div className="md:col-span-7 p-8 sm:p-12 flex flex-col justify-center">
          <div className="mb-6">
            <span className="text-[9px] font-bold text-neutral-400 uppercase tracking-[0.25em] font-sans">NEW CUSTOMER</span>
            <h3 className="text-xl font-black font-sans uppercase mb-1 text-black tracking-wide mt-1">Create Account</h3>
            <p className="text-neutral-500 text-xs mb-2">Register to unlock exclusive brand benefits.</p>
            <Link href="/signup/merchant" className="text-[10px] text-stone hover:text-black font-bold uppercase tracking-widest transition-colors flex items-center gap-1.5 mt-1 pb-1 border-b border-black/10 w-fit">
              Are you a designer? Register as a Merchant <i className="bi bi-arrow-right"></i>
            </Link>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="text-[10px] uppercase font-bold text-neutral-500 block mb-1.5 font-sans tracking-wider">Location / Region</label>
              <input
                type="text"
                required
                value={location}
